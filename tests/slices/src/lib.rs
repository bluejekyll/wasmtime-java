use wasmtime_jni_exports::{Borrowed, Owned, WasmAllocated, WasmSlice};
...
pub extern "C" fn say_hello_to_java() {
    let hello: Borrowed<WasmSlice> = "Hello Java!".into();

    unsafe { hello_to_java(hello.ptr(), hello.len()) }
}
